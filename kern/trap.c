...
uint32 clockalgorithm (struct Env *curenv, uint32 fault_va)
{
	int i=0;
	int pos=curenv->page_last_WS_index;
    int count =0;

    // try 1
	while(1==1)
	{
      	uint32 current_page=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
	   	uint32 perms=pt_get_page_permissions(curenv,current_page);
	    uint32 usedbit=perms&PERM_USED;
    	uint32 modified=perms&PERM_MODIFIED;

    	if(usedbit==0 && modified ==0)
	      {
	            return current_page;
	      }
    	...
    	if(curenv->page_last_WS_index == pos)
    		break;
	}
   ...
   {
	  uint32 current_page=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
	  unsigned int perms=pt_get_page_permissions(curenv,current_page);
	  int usedbit=perms&PERM_USED;
	   ...
	   if(usedbit==0)
		...
		pt_set_page_permissions(curenv,current_page,0,PERM_USED);
		curenv->page_last_WS_index += 1;
		if(curenv->page_last_WS_index == curenv->page_WS_max_size)
			curenv->page_last_WS_index = 0;
	...
	return clockalgorithm(curenv, fault_va);
}


void placement(struct Env *curenv, uint32 fault_va)
{
	struct Frame_Info *vaframe=NULL;
	uint32 *ptr_table=NULL;

	vaframe=get_frame_info(curenv->env_page_directory,(void *)fault_va,&ptr_table);

	int buffered=0;

	if(ptr_table!=NULL)
	        buffered=ptr_table[PTX(fault_va)]&PERM_BUFFERED;

	if(buffered!=0)
	{
		pt_set_page_permissions(curenv,fault_va,PERM_PRESENT,PERM_BUFFERED);
		int modifiedbit=ptr_table[PTX(fault_va)]&PERM_MODIFIED;

    	vaframe->isBuffered=0;

    	struct Linked_List *ptr=&modified_frame_list;

    	if(modifiedbit!=0)
    	{
   			bufferlist_remove_page(ptr,vaframe);
   		}
	...
	else
	{
		...
		if(vaframe==NULL)
		{
		  ...
		  ret=map_frame(curenv->env_page_directory,newframe,(void *)fault_va,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
		}
 	    ...
 	    if(ret==E_PAGE_NOT_EXIST_IN_PF)
          {
     	    ...
     	    {
     	    	panic(" Invalid Access Neither stack Page Nor exist in page file ");
     	    }
          }
   }
...
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va)
{
	...
	// Write your code here, remove the panic and write your code
		...
		uint32 currentsize=env_page_ws_get_size(curenv);
		uint32 max_num_pages=curenv->page_WS_max_size;

		if(currentsize<max_num_pages)
		{
		...
		{
			uint32 virtualaddress=clockalgorithm(curenv,fault_va);
			uint32 *ptr_table=NULL;
			struct Frame_Info *frame=NULL;
			frame =get_frame_info(curenv->env_page_directory,(void *)virtualaddress,&ptr_table);

			frame->isBuffered=1;
			frame->environment=curenv;
			frame->va=virtualaddress;

			pt_set_page_permissions(curenv,virtualaddress,PERM_BUFFERED,PERM_PRESENT);

			int modified=ptr_table[PTX(virtualaddress)]&PERM_MODIFIED;

			struct Linked_List *ptr=&free_frame_list;

			if(modified==0)
				bufferList_add_page(ptr,frame);
			else
			{
				ptr=&modified_frame_list;
				bufferList_add_page(ptr,frame);
				int count=getModifiedBufferLength();
				if(ptr->size>=count)
				{
					struct Frame_Info *iterator=NULL;
					struct Linked_List *list=&modified_frame_list;
					struct Linked_List *freelist=&free_frame_list;
					LIST_FOREACH(iterator,list)
					{
						struct Env* env=iterator->environment;
						uint32 *va=(uint32 *)iterator->va;
						pf_update_env_page(env,va,iterator);
						pt_set_page_permissions(iterator->environment,iterator->va,0,PERM_MODIFIED);
						bufferList_add_page(freelist,iterator);
					}
