...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
    ...
    if(sharedcount==MAX_SHARES)
    	return E_NO_SHARE;

	int Id=allocate_share_object(&newshare);
	    sharedcount++;

	unsigned int num_frame=ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;

	int ret;
	for(int i=0;i<num_frame;i++)
	{
	    struct Frame_Info *newframe=NULL;
	    ret=allocate_frame(&newframe);

	    if(ret==E_NO_MEM)
		    return E_NO_MEM;

        ret=map_frame(myenv->env_page_directory,newframe,(void *)virtual_address,PERM_WRITEABLE|PERM_USER|PERM_USED);

        if(ret==E_NO_MEM)
        		return E_NO_MEM;

    	add_frame_to_storage(newshare->framesStorage,newframe,i);
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	struct Share *shared_obj=&shares[ID];

	unsigned int num_pages=ROUNDUP(shared_obj->size,PAGE_SIZE)/PAGE_SIZE;

	for(int i=0;i<num_pages;i++)
	{
		struct Frame_Info *frame=NULL;
		frame=get_frame_from_storage(shared_obj->framesStorage,i);
		int permwritable=0;
		int ret;
		if(shared_obj->isWritable==1)
		{
		  ret=map_frame(myenv->env_page_directory,frame,(void *)virtual_address,PERM_WRITEABLE|PERM_USER|PERM_USED);
		}
		else
		{
	      ret=map_frame(myenv->env_page_directory,frame,(void *)virtual_address,PERM_USER|PERM_USED);
		}
