...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 *startadd=kmalloc(4*1024);

	uint32 physicaladdress=kheap_physical_address((unsigned int)startadd);

	physicaladdress = physicaladdress|PERM_USER|PERM_WRITEABLE|PERM_PRESENT;

	uint32 direntry=PDX(virtual_address);
	ptr_page_directory[direntry]=physicaladdress;
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(int i=0;i<num_pages;i++)
	{
	   ...
	   uint32 perms=pt_get_page_permissions(e,virtual_address);

	   int buffered=perms&PERM_BUFFERED;
	   int modified=perms&PERM_MODIFIED;

	   if(buffered !=0)
	   {
		   struct Frame_Info *frame=NULL;
		   frame=get_frame_info(e->env_page_directory,(void *)virtual_address,&ptr_table);
		   if(modified != 0)
		   {
			   struct Linked_List *list=&modified_frame_list;
			   bufferlist_remove_page(list,frame);
			   frame->isBuffered=0;
		   ...
		   else
		   {
			   frame->isBuffered=0;
			   frame->environment=NULL;
			   struct Linked_List *list=&free_frame_list;
			   bufferlist_remove_page(list,frame);
		   }
		   free_frame(frame);
	    ...
	    for(int i=0;i<e->page_WS_max_size;i++)
	     {
	   		  ...
	   		  {
	   			get_page_table(e->env_page_directory,(void *)virtual_address,&ptr_table);
	    ...
	    for(int i=0;i<1024 && ptr_table!=NULL;i++)
	    {
	    	int present=ptr_table[i]&PERM_PRESENT;
	    	if(present!=0)
	    	{
	    		delete=0;
	    		break;
	    ...
	    if(delete==1)
	    {
	    	...
	    	unmap_frame(e->env_page_directory,ptr_table);
	    }
